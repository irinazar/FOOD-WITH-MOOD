...
import { getOneRestCommentThunk } from '../../../features/redux/slices/oneRestCommentSlice/oneRestCoomentThunk';

export default function CommentSection(): JSX.Element {
  const { id } = useParams();
  const dispatch = useAppDispatch();
  const { oneRestComments } = useAppSelector((state) => state.oneRestComments);

  console.log(oneRestComments, 'AAAAAAAAAAAAAAAAAAAAAAAAAAA');
  

  useEffect(() => {
    void dispatch(getOneRestCommentThunk(Number(id)));
  }, []);
  return (
